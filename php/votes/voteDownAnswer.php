    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (isset($_SESSION['id_user'])) {
            ...
            if (isset($_POST['id_reponse'])) {
                ...
                // Check
                $stmtCHECK = $conn->prepare('SELECT * FROM `downvotereponse`
                ...
                if ($stmtCHECK->rowCount() > 0) {
                    $stmtDELETE = $conn->prepare('DELETE FROM `downvotereponse`
                    WHERE `idutilisateur` = :idutilisateur
                    AND `idreponse` = :idreponse');
                    ...
                    $stmtINSERT = $conn->prepare('INSERT INTO `downvotereponse` 
                    ...
                    $stmt = $conn->prepare('SELECT * FROM `votereponse`
                    WHERE idutilisateur = :idutilisateur 
                    AND idreponse = :idreponse');
                    ...
                    if ($count > 0) {
                        $stmt2 = $conn->prepare('DELETE FROM `votereponse`
