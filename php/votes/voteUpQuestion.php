    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (isset($_SESSION['id_user'])) {
            ...
            if (isset($_SESSION['id_question'])) {
                ...
                // Check
                $stmtCHECK = $conn->prepare('SELECT * FROM `votequestion`
                ...
                if ($stmtCHECK->rowCount() > 0) {
                    $stmtDELETE = $conn->prepare('DELETE FROM `votequestion`
                    WHERE `idutilisateur` = :idutilisateur
                    AND `idquestion` = :idquestion');
                    ...
                    $stmtINSERT = $conn->prepare('INSERT INTO `votequestion` 
                    ...
                    $stmt = $conn->prepare('SELECT * FROM `downvotequestion`
                    WHERE idutilisateur = :idutilisateur 
                    AND idquestion = :idquestion');
                    ...
                    if ($count > 0) {
                        $stmt2 = $conn->prepare('DELETE FROM `downvotequestion`
